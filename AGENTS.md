...
6. **Git Operations & Authorization**: The AI agent is authorized to execute `git add` and `git commit` for verified code changes. However, `git push` is strictly prohibited and MUST be performed manually by the user.
7. **Node.js & Terminal Command Authorization**: The AI agent is pre-authorized to run standard Node.js and NPM execution, testing, build, and installation commands (`node`, `npm run dev`, `npm run build`, `npm test`, `npm install`, `npx`, etc.) for building, running dev servers, testing, and debugging without prompting for routine shell execution.
