...
export default function App() {
  ...
  // Open Delete Tenant Confirmation Modal
  const openDeleteTenantModal = (tId: string, companyName: string) => {
    setDeleteTargetTenant({ tenantId: tId, companyName });
    setDeleteModalError(null);
  };

  // Confirm & Execute Permanent Tenant Deletion
  const confirmPermanentDelete = async () => {
    if (!token || !deleteTargetTenant) return;
    setIsDeletingTenant(true);
    setDeleteModalError(null);

    const { tenantId, companyName } = deleteTargetTenant;
    ...
    try {
      ...
      setOnboardSuccess(`Tenant "${companyName}" (${tenantId}) has been permanently deleted.`);
                              ...
                              <tr className={`hover:bg-[#F2EFE7]/60 transition-colors ${isExpanded ? 'bg-[#F2EFE7]/40' : ''}`}>
                                ...
                                <td className="p-3.5 text-right whitespace-nowrap">
                                  <div className="flex items-center justify-end gap-2">
                                    ...
                                    <button
                                      onClick={() => openDeleteTenantModal(t.tenantId, t.companyName)}
